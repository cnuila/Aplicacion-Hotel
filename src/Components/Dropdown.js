            ...
            <div>
                <div class="relative inline-block text-left">
                    <div>
                        ...
                        {firebase.auth().currentUser ? <div>{firebase.auth().currentUser.displayName}</div> : <div>Perfil</div>}
                    ...
                    <div class="origin-top-right z-40 absolute lg:right-0 mt-2 w-48 rounded-md shadow-lg bg-blue-900 ring-1 ring-black ring-opacity-5">
                        <div class={(this.state.open ? "py-1" : "hidden")}>
                            <Link to="/login" class={(firebase.auth().currentUser ? "hidden" : "block px-4 py-2 text-sm text-white border-b-2 border-transparent hover:border-yellow-200")} role="menuitem">Iniciar Sesión</Link>
                            <Link to="/" class={(firebase.auth().currentUser ? "block px-4 py-2 text-sm text-white border-b-2 border-transparent hover:border-yellow-200" : "hidden")} role="menuitem" onClick={this.cerrarSesion}>Cerrar Sesión</Link>
